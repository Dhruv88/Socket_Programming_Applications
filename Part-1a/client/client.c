    ...
    if(rcvlen!=-1)
    {
      ...
      if(rcvKit->rcvTimes[id]!=-2)
      {
        ...
        else
        {
          ...
          inet_ntop(AF_INET,&(((struct sockaddr_in*)rcvKit->from)->sin_addr), ip4,INET_ADDRSTRLEN);
...
int main(int argc, char* argv[]){
  ...
  initializeEchoSendKit(interval, packetSize, numberOfMessages, servinfo->ai_addr, servinfo->ai_addrlen, socketDescriptor, sendTimes, sendKit);

  initializeEchoRcvKit(interval, packetSize, numberOfMessages, servinfo->ai_addr, servinfo->ai_addrlen, socketDescriptor, sendTimes, rcvTimes, timeOut, rcvKit);
